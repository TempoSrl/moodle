SET @column_exists = (SELECT COUNT(*) 
                      ...
                      WHERE TABLE_SCHEMA = 'moodle' 
                        AND TABLE_NAME = 'mdl_quiz_attempts' 
                        AND COLUMN_NAME = 'action');

SET @sql = IF(@column_exists = 0, 
              'ALTER TABLE `moodle`.`mdl_quiz_attempts` ADD COLUMN `action` BIGINT NULL AFTER `gradednotificationsenttime`;', 
              'SELECT "Column already exists. It'' all OK";');

PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

DROP VIEW IF EXISTS question_attempt_view;

CREATE  VIEW question_attempt_view AS
WITH times AS (
    SELECT 
  		QA.id,
        Quiz_AT.id as attempt_id,
        QQ.course,
        Quiz_AT.action,
        Quiz_AT.userid,
		QU.id as usage_id,
        ...
        -- Calcolo 'next_page' su quella domanda
		(SELECT FROM_UNIXTIME(QA2.value) 
		 FROM mdl_question_attempt_step_data QA2 
				JOIN mdl_question_attempt_steps QAS2 ON QA2.attemptstepid = QAS2.id
				WHERE QAS2.questionattemptid IN (
						SELECT QA3.id 
							FROM mdl_question_attempts QA3 
							WHERE QA3.questionusageid = QA.questionusageid
							AND QA3.id > QA.id
				)
		 AND QA2.name = 'next_page_timestamp'
		 ORDER BY QAS2.id ASC
		 LIMIT 1) as next_page_time
    ...
    FROM 
        ...
        JOIN mdl_quiz_attempts Quiz_AT ON Quiz_AT.uniqueid = qu.id
        join mdl_quiz qq on Quiz_AT.quiz = qq.id
...
SELECT 
    ...
    usage_id, question_id,  question,  resp,  _right, result, ask_time, resp_time, next_page_time,
